...
def load_trees_csv(output_dir, dataset = ""):
    data = {k: {} for k in ("train", "test")}
    data["dataset"] = dataset
    for k in ("train", "test"):
        fn = Path(output_dir) / f"{k}.csv"
        if not fn.exists():
            print(f"missing {fn}")
            continue
        data[k] = pd.read_csv(fn).to_dict(orient="list")
...
def predictions_to_dict(preds, fn):
    if "e" in preds[0]:
    ...
    if 'sexist' in examples[0]:
        d["label"] = [e["sexist"] for e in examples]
    else:
...
# For our convenience we are changing it so as to check the training data correlations
def load_trees_from_predictions(output_dir, dataset=""):
    data = {k: {} for k in ("train", "test")}
    data["dataset"] = dataset
    for k, name in (("train", f"{dataset}_train"), ("test", f"{dataset}")):
        ...
        with open(fn, "r") as f:
            preds = json.load(f)
        data[k] = predictions_to_dict(preds, dataset)
        data[k]["tree"] = tree_utils.strings_to_trees(data[k]["tree"])
        if dataset == "imdb":
            data[k] = combine_imdb_sents(data[k])
        data[k]["y"] = np.array(data[k]["label"])
...
#Adding this function since we are only interested in the train data correlations
def add_features_train(data, name, func, key = "tree", verbose = True, sort = True):
    if "vect" not in data:
        data["vect"] = {}
        data["idx_w"] = {}
    if 'features' not in data:
        data['features'] = {}
    data["features"][name] = [func(x) for x in data[key]]
    ...
    if "X" not in data:
        data["X"] = {}
    features = data['features'][name]
    data["X"][name] = vect.transform(features)
    X, y = data["X"][name], data["y"]
...
def get_subtree_feature_table_train(data, name="Subtrees", label_name = ""):
    mi, counts, idx_w = (
        data["mi"][name],
        data["counts"][name],
        data["idx_w"][name],
    )
    label_names = data_utils.get_label_names(label_name)
...
def get_merged_feature_table_train(data, merge_name="merge", k=8, label_name = 'sexism'):
    mi, counts, idx_w = (
        data["mi"][merge_name],
...
def add_merges_train(
    data, feature_name, merge_name = "merge", K = 1000,
    filter_ = None, by_argmax = True, by_template = False):
    if merge_name not in data:
        data[merge_name] = {feature_name: {}}
    key = feature_name
    if key not in data[merge_name]:
        data[merge_name][key] = {}
    X = data["X"][feature_name]
    y = data["y"]
    ...
    data["X"][merge_name] = np.array([v[3] for _, v in items]).T
