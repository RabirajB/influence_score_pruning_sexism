...
from utils import data_utils, logging, metrics, model_utils, tokenizers
from grammars import pcfg, spcfg
...
def get_datasets(args, tokenizer):
    ...
    for name in args.eval_on:
        eval_datasets[name] = data_utils.load_dataset(
            name,
            "" if (name.endswith(".json") or name.endswith(".tsv"))  or name.endswith('.csv') else "test",
            tokenizer,
            **vars(args),
        )
...
#For this run we will just use the BERt pre-trained tokenizer
def run(args):
